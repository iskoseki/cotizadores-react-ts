...
export interface Props {
  ...
  campos_ocultos_formulario: [];
  campos_formulario_autoimpulsa: visiblesType[];
  campos_formulario: visiblesType[];
  titulo_formulario: string;
}

export const useFormValues = () => {
  const { data } = useFetch<Props>(
    "https://www.montepio.org.mx/wp-json/acf/v3/pages/36"
  );
  ...
  useEffect(() => {
    if (data) {
      setOcultos(data.campos_ocultos_formulario || []);
      setOcultosAuto(data.campos_ocultos_formulario_autoimpulsa || []);
      setVisibles(data.campos_formulario || []);
